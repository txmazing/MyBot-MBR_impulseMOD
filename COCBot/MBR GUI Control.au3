...
Func GUIControl_WM_NOTIFY($hWind, $iMsg, $wParam, $lParam)
	...
	Switch $nID
		...
		Case $g_hGUI_THSNIPE_TAB
			tabTHSnipe()
		Case $g_hGUI_MOD_TAB ; ADDED By IMMOD
			tabMOD()
		Case $g_hGUI_BOT_TAB
			tabBot()
		Case Else
...
Func BotGuiModeToggle()
	...
	Switch $g_iGuiMode
		...
		Case 1
			...
			GUICtrlDelete($g_hTabMOD) ; ADDED By IMMOD
			...
			GUICtrlDelete($g_hGUI_MOD_TAB) ; ADDED By IMMOD
		...
		Case 2
			...
			; refresh tab states
			tabMOD() ; ADDED By IMMOD
			tabBot()
			tabDONATE()
			tabSEARCH()
			tabAttack()
			tabVillage()
			;tabSwitchOptions() ; ADDED By IMMOD
...
Func tabMain()
	...
	Select
		Case $tabidx = 0 ; Log
			...
			GUISetState(@SW_HIDE, $g_hGUI_MOD) ; ADDED By IMMOD
		...
		Case $tabidx = 3 ; MOD
			...
			tabMOD()
			;------------------ADDED By IMMOD - END------------------
		Case $tabidx = 4 ; Options ; EDITED By IMMOD
			GUISetState(@SW_HIDE, $g_hGUI_LOG)
			GUISetState(@SW_HIDE, $g_hGUI_VILLAGE)
			GUISetState(@SW_HIDE, $g_hGUI_ATTACK)
			GUISetState(@SW_HIDE, $g_hGUI_MOD) ; ADDED By IMMOD
		...
		Case $tabidx = 5 ; About ; EDITED By IMMOD
			GUISetState(@SW_HIDE, $g_hGUI_LOG)
			GUISetState(@SW_HIDE, $g_hGUI_VILLAGE)
			GUISetState(@SW_HIDE, $g_hGUI_ATTACK)
			GUISetState(@SW_HIDE, $g_hGUI_MOD) ; ADDED By IMMOD
...
Func Bind_ImageList($nCtrl, ByRef $hImageList)
	...
	Switch $nCtrl
		Case $g_hTabMain
			...
			Local $aIconIndex = [$eIcnHourGlass, $eIcnTH12, $eIcnAttack, $eIcnIMMod, $eIcnGUI, $eIcnInfo] ; EDITED By IMMOD
		...
		Case $g_hGUI_MISC_TAB
			Local $aIconIndex = [$eIcnTH1, $eIcnBuilderHall, $eIcnClanGames] ; EDITED By IMMOD
